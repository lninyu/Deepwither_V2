...
> [!WARNING]
> **1. @Injectアノテーションの必須化とフィールドインジェクションの禁止**
> 依存関係（他クラスのインスタンスなど）を受け取るコンストラクタには、**必ず `@Inject` アノテーションを付与**してください。付与し忘れると「No @Inject constructor found」というエラーで起動に失敗します。
> また、本プロジェクトでは設計の明確化と安全性の確保のため、フィールドインジェクション（フィールドに直接 `@Inject` を付けること）はサポートしていません。
